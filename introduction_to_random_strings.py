...
. In this graph, we can see that the logarithm of x-values between 0 and 1 always winds up mapping to y-values between −∞
 and 0: x-values near 0 have logarithms close to −∞, and x-values close to 1 have logarithms close to 0.
...
# Define variables
print("Introduce string s:")
dna = input()
print("Introduce array A:")
A = input()
A = A.split(" ")
...
B = []
for p in A:
    probs = {"A":((1-p) / 2), "C":(p / 2), "G":(p / 2), "T":((1-p) / 2)}
    log_sum = np.sum(np.log10([probs[x] for x in dna]))
    B.append(log_sum)
